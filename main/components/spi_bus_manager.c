...
spi_bus_config_t spi3_bus_config;
...
esp_err_t init_spi_bus_manager(void) {
    ...
    ret = spi_bus_initialize(SPI2_HOST, &spi2_bus_config, SPI_DMA_CH_AUTO);
    if (ret != ESP_OK) {
    ...
    ret = spi_bus_initialize(SPI3_HOST, &spi3_bus_config, SPI_DMA_CH_AUTO);
    if (ret != ESP_OK) {
